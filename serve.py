...
# 1. Sınıf isimleri (Notebook'taki sırayla tam olarak aynı olmalı)
class_names = ['adenocarcinoma', 'large.cell.carcinoma', 'normal', 'squamous.cell.carcinoma']

# 2. Modeli yükle
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
model = ChestCancerModel(num_classes=4)
# Notebook'ta kaydettiğin dosya ismiyle aynı olmalı
...
def predict_lung_cancer(inp_img):
    ...
    # Görseli modelin beklediği formata sok (Notebook'taki transform ile aynı)
...
if __name__ == "__main__":
    interface.launch(share=True) # share=True dersen hocana atabileceğin 72 saatlik bir link de verir